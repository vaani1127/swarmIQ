...
| **Microsoft Foundry** (`swarmiq-openai-vaani`) | Azure OpenAI / Foundry resource provisioned in `swarmiq-rg` (Central India) as the documented production-grade alternative to GitHub Models. Switching from GitHub Models to direct Azure OpenAI requires: (1) deploying a model in the Foundry resource, (2) updating three env vars (`LLM_BASE_URL`, `LLM_API_KEY`, `LLM_MODEL`) in Key Vault, (3) triggering a Container App revision. **Zero code changes** | The Microsoft enterprise inference path is provisioned and verifiable in our Azure subscription; we ship on the free-tier GitHub Models endpoint for the hackathon, with the Foundry resource standing by for production scale |
